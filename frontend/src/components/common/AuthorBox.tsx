...
export default function AuthorBox({ author }: AuthorBoxProps) {
  ...
  return (
    <Link 
      ...
      className="block relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#1B2B3B] to-[#121C26] border border-white/10 p-6 sm:p-8 mt-12 group transition-all duration-300 hover:border-[#FF8C00]/40 hover:shadow-[0_0_30px_rgba(255,140,0,0.15)] cursor-pointer"
    >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 relative z-10">
        ...
        <div className="relative shrink-0">
          <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-[#FF8C00]/20 relative z-10 bg-[#0F1720] transition-colors duration-300 group-hover:border-[#FF8C00]/50">
            {author.photo?.url ? (
              <Image 
                src={imageUrl} 
                alt={author.name} 
        ...
        <div className="flex-1">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
            <div>
              <h3 className="text-xl font-bold text-white mb-1 group-hover:text-[#FF8C00] transition-colors">{author.name}</h3>
              {author.credentials && (
            ...
            <span className="inline-flex items-center gap-2 text-sm font-medium text-white/70 group-hover:text-[#FF8C00] transition-colors whitespace-nowrap">
              View Full Profile

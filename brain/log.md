...
## 2026-09-21
- Синхронизация отображения дат блога (`createdAt`), сортировка по дате добавления и вывод метаданных автора и категории.
    - **Blog Detail Hero Date Fix**: На детальной странице блога (`src/app/blog/[slug]/page.tsx`) в hero-секции отображение даты переведено с `post.publishedAt` на `post.createdAt`, предотвращая изменение даты при повторном сохранении/редактировании статьи в Strapi CMS и приводя поведение к общему списку блога. [[frontend/blog-date-sorting-metadata]] [[frontend/blog-ui]]
    - **Homepage Blog Section Sorting (`createdAt:desc`)**: В сервисе выборки статей `src/services/post.service.ts` параметр сортировки изменен с `sort=publishedAt:desc` на `sort=createdAt:desc`. Теперь первыми на главной отображаются статьи, созданные последними. [[frontend/blog-date-sorting-metadata]] [[frontend/data-fetching-pattern]]
    - **Homepage Blog Author & Date Display**: В компоненте `LatestPosts.tsx` на главной исправлена проблема пустой иконки автора: в `post.service.ts` добавлен `populate[1]=author`, а в разметке добавлена защита `{post.author?.name && (...)}`. Отображение даты унифицировано на `createdAt` в формате `en-US` (`Month Day, Year`). Также `populate` автора добавлен в `blog/page.tsx`. [[frontend/blog-date-sorting-metadata]] [[frontend/authors-system]]
    - **Related Articles Date & Category**: В блоке похожих статей (`RelatedArticles` / `RelatedArticle.tsx`) добавлен вывод даты создания статьи (`Calendar` + `post.createdAt`) и её категории (`Tag` + `post.category.name`). В `getRelatedBlogPosts` (`/blog/[slug]/page.tsx`) добавлены `populate[3]=category` и сортировка `sort=createdAt:desc`. [[frontend/blog-date-sorting-metadata]] [[frontend/blog-ui]]

## 2026-09-17
- Реализация системы социального шеринга, устранение бага генерации Twitter Card и интерактивная якорная навигация в тренировках.
    - **Universal ShareButtons Component**: Разработан компонент `ShareButtons.tsx` с поддержкой шеринга в X (Twitter), Facebook, нативного меню смартфонов (Web Share API) и копирования ссылки в буфер обмена (`navigator.clipboard` с анимированным статусом «Copied!» и изумрудным акцентом). Дизайн выполнен в стиле glassmorphism с ambient-свечением и hover-микроанимациями. Блок интегрирован в детальные статьи блога (`/blog/[slug]`) и карточки тренировок (`/workouts/[slug]`). [[frontend/social-share-and-anchors]] [[frontend/blog-ui]]
    - **Twitter Card Preview & Absolute Protocol Fix**: Устранена проблема неподтягивающегося изображения при шеринге ссылки в Twitter (`No IMAGE_SRC has been found`). Выяснено, что Twitterbot требует строгий абсолютный HTTPS-адрес. В `utils/image.ts` и `lib/utils.ts` fallback-хост Strapi переведён с `http://localhost:1337` на `https://api.fitway.best`, а в `generateMetadata` для блога и тренировок добавлена валидация протокола, размеры `1200x630`, `secureUrl` и `alt`. [[frontend/social-share-and-anchors]] [[frontend/seo]] [[deploy/strapi-media-production]]
    - **Interactive Anchor Links in WorkoutHero**: В шапке детальной страницы тренировки (`WorkoutHero.tsx`) кнопка "START WORKOUT" переведена на плавный якорь `href="#first-exercise"` к первому упражнению в `ExerciseList.tsx` (`scroll-mt-28`), кнопка "SHARE" переведена на якорь `href="#share-section"` к блоку соцсетей, а неиспользуемая кнопка "SAVE" удалена. В `globals.css` подключено глобальное правило `html { scroll-behavior: smooth; }`. [[frontend/social-share-and-anchors]] [[frontend/responsive-layout]]
...
- Автоматическая обработка внешних ссылок и E-E-A-T цитирование первоисточников в статьях блога (`/blog/[slug]`).
    - **External Link Component in ReactMarkdown**: В `src/app/blog/[slug]/page.tsx` добавлен кастомный компонент тега `<a>` для `ReactMarkdown`. Внешние ссылки (`http://`, `https://`) автоматически получают `target="_blank"` и защитные атрибуты `rel="noopener noreferrer"`, предотвращающие уязвимость tabnabbing и сохраняющие пользователя на странице. Внутренние ссылки маршрутизируются через Next.js `<Link>`. Ссылки остаются follow (dofollow) для поисковых ботов, передавая авторитетные сигналы цитирования первоисточников (PubMed, NCBI) согласно правилам Google E-E-A-T. [[frontend/external-links-eeat]] [[frontend/markdown-tables-gfm]] [[frontend/seo]] [[roadmap/adsense-approval-plan]]
...
- Устранение ошибки индексации страниц категорий блога в Google Search Console («Вариант страницы с тегом canonical»).
    ...
    - **Self-referential Canonical & OpenGraph**: В `src/app/blog/category/[slug]/page.tsx` добавлен явный самореферентный canonical URL (`alternates: { canonical: /blog/category/${params.slug} }`) и OpenGraph метаданные, устраняющие некорректную каноникализацию. [[bugs/canonical-tag-inheritance]] [[frontend/seo]]
    - **Dynamic Categories in Sitemap**: В генератор карты сайта `src/app/sitemap.ts` добавлен вызов Strapi API (`/api/categories`) для динамического включения всех страниц категорий в `sitemap.xml` (`weekly`, `priority: 0.7`). [[bugs/canonical-tag-inheritance]] [[frontend/seo]]
- Комплексный аудит Google AdSense («Бесполезный контент / Low value content») и внедрение стандартов E-E-A-T.
    - **Editorial Guidelines & Review Standards (`/editorial-policy`)**: Разработана отдельная страница редакционных стандартов с регламентом 4-этапной верификации контента (Sourcing, Drafting, Peer Review, Auditing), ссылками на научные базы (PubMed, ISSN, NIH, ACSM), презентацией команды экспертов и прозрачной политикой исправлений с медицинским дисклеймером и JSON-LD `WebPage`. [[frontend/editorial-policy]] [[frontend/seo]]
    - **Peer-Review Architecture & UI (`ReviewedByBox`)**: Устранена проблема саморецензирования путем разделения ролей автора (`author`) и рецензента (`reviewedBy`). Создан компонент `ReviewedByBox.tsx` с изумрудным световым акцентом (`#00C853`), бейджем подлинности, автоматическим определением типа рецензии (`Medically Reviewed` vs `Scientifically Reviewed`), датой проверки и ссылкой на стандарты. На странице `/blog/[slug]` добавлен бейдж рецензента в hero-блоке и блок верификации перед карточкой автора. [[frontend/editorial-policy]] [[frontend/authors-system]] [[frontend/blog-ui]]
    - **Footer & Social Trust Signals**: В подвале сайта (`Footer.tsx`) раздел ссылок обновлен на "Legal & Trust", добавлена ссылка на `/editorial-policy`. Заглушки `href="#"` заменены на реальные страницы сообществ FitWay в Facebook, Instagram и Twitter/X. [[frontend/editorial-policy]] [[frontend/seo]]
    - **Sitemap Inclusion**: Маршрут `/editorial-policy` добавлен в статические страницы `sitemap.ts` (`monthly`, `priority: 0.6`). [[frontend/editorial-policy]] [[frontend/seo]]

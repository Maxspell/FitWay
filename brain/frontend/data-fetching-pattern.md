...
## The Pattern
1. **Service Layer**: Extract data fetching logic (e.g., `getBlogPosts`, `getWorkouts`) into `src/services/*.service.ts`. This encapsulates Strapi API tokens, fetch options, cache rules (`revalidate`), and complex sorting/filtering queries (e.g., `sort=createdAt:desc`, `populate[1]=author`).
...
**Related:**
- [[frontend/blog-date-sorting-metadata]] — Синхронизация дат, сортировки (`sort=createdAt:desc`) и populate в запросах к Strapi.
- [[frontend/blog-ui]] — Компоненты карточек блога.
